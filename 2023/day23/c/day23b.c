...
static bool graph_has_edge(struct Graph* G, uint8_t u, uint8_t v) {
  for (int i = 0; i < 4; ++i) {
    if (G->E[u][i] == v) {
      return true;
    }
...
static void graph_make_bidirectional(struct Graph* G) {
  for (uint8_t u = 0; u < G->num_nodes; ++u) {
    for (int i = 0; i < 4; ++i) {
      uint8_t v = G->E[u][i];
      if (v == GRAPH_UNNEIGHBOR)
        break;
      if (graph_has_edge(G, v, u)) {
        assert(G->W[u][v] == G->W[v][u]);
      }
      else {
        graph_add_edge(G, v, u, G->W[u][v]);
      }
    }
  }
}
