...
static uint16_t P[MAX_PARTS][4];
static uint8_t num_parts = 0;
...
uint32_t solve(void) {
  ...
  for (i = 0; i < num_parts; ++i) {
    xmas = P[i];
    w = hashmap_find(&name_to_workflow, in_state_string);
    while (true) {
      if (w == accept_state || w == reject_state)
        break;
      for (j = 0; j < w->num_rules; ++j) {
        if (w->rules[j].compare == COMPARE_LESS_THAN) { 
          if (xmas[w->rules[j].variable] < w->rules[j].value) {
            w = w->rules[j].next_state;
            break;
          }
        }
        else if (w->rules[j].compare == COMPARE_GREATER_THAN) {
          if (xmas[w->rules[j].variable] > w->rules[j].value) {
            w = w->rules[j].next_state;
            break;
          }
        }
        else if (w->rules[j].compare == COMPARE_NIL) {
          w = w->rules[j].next_state;
          break;
        }
        else {
